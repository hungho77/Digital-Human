...
def video2imgs(vid_path, save_path, ext='.png', cut_frame=10000000):
    ...
    while True:
        ...
        ret, frame = cap.read()
        if ret:
            cv2.putText(frame, "Digital Human", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (128,128,128), 1)
...
##todo Simple judgment based on file extension, for more precise results you can modify yourself using magic
def is_video_file(file_path):
    video_exts = ['.mp4', '.mkv', '.flv', '.avi', '.mov']  # Here are some common video file extensions, you can add more as needed
    file_ext = os.path.splitext(file_path)[1].lower()  # Get file extension and convert to lowercase
...
def create_musetalk_human(file, avatar_id):
    # Save file settings, can be left unchanged
